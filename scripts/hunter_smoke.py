...
Chromium headless + stealth con session.json y abre la home de la tienda FIFA.
...
from core.HunterService import TICKETS_HOME_URL, HunterService
...
async def _browser_smoke(session_path: Path) -> None:
...
def main() -> None:
    ...
    asyncio.run(_browser_smoke(session_path))
